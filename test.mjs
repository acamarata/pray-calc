...
describe('getAngles — dynamic depression', () => {
  it('returns object with fajrAngle and ishaAngle', () => {
    const a = getAngles('2024-06-21', 40.7, -74.0);
...
describe('MSC minute offsets', () => {
  ...
  it('getMscFajr increases with latitude (summer)', () => {
    const m30 = getMscFajr('2024-06-21', 30);
    const m50 = getMscFajr('2024-06-21', 50);
    assert(m50 > m30, `Expected lat50 (${m50}) > lat30 (${m30})`);
  });

  it('getMscFajr equator ~75 minutes year-round', () => {
    const summer = getMscFajr('2024-06-21', 0);
    const winter = getMscFajr('2024-12-21', 0);
  ...
  it('getMscIsha shafaq modes return different values at high lat', () => {
    const general = getMscIsha('2024-06-21', 51.5, 'general');
    const ahmer   = getMscIsha('2024-06-21', 51.5, 'ahmer');
    const abyad   = getMscIsha('2024-06-21', 51.5, 'abyad');
...
describe('getTimes — structure', () => {
  it('returns all required fields', () => {
    const t = getTimes('2024-06-21', 40.7, -74.0);
  ...
  it('chronological order', () => {
    const t = getTimes('2024-06-21', 40.7, -74.0, -4);
...
describe('getTimes — geographic scenarios', () => {
  ...
  it('Makkah summer solstice — Sunrise ~05:39', () => {
    const t = getTimes('2024-06-21', 21.4225, 39.8262, 3);
  ...
  it('New York winter solstice — Sunrise ~07:20', () => {
    const t = getTimes('2024-12-21', 40.7128, -74.0060, -5);
  ...
  it('London summer — Sunrise ~04:43', () => {
    const t = getTimes('2024-06-21', 51.5074, -0.1278, 1);
  ...
  it('Sydney summer (Gregorian Jan) — Sunrise ~06:00', () => {
    const t = getTimes('2024-01-15', -33.8688, 151.2093, 11);
  ...
  it('Singapore — all times finite', () => {
    const t = getTimes('2024-06-21', 1.3521, 103.8198, 8);
...
describe('getTimes — seasonal variation', () => {
  it('NY Sunrise earlier in summer than winter', () => {
    const summer = getTimes('2024-06-21', 40.7, -74.0, -4).Sunrise;
    const winter = getTimes('2024-12-21', 40.7, -74.0, -5).Sunrise;
    assert(summer < winter, `Summer ${summer} < Winter ${winter}`);
  });

  it('NY Sunset later in summer than winter', () => {
    const summer = getTimes('2024-06-21', 40.7, -74.0, -4).Maghrib;
    const winter = getTimes('2024-12-21', 40.7, -74.0, -5).Maghrib;
    assert(summer > winter, `Summer ${summer} > Winter ${winter}`);
  });

  it('Noon time consistent across seasons (same tz, within 30 min)', () => {
    const base = getTimes('2024-06-21', 40.7, -74.0, -5).Noon;
  ...
  it('Fajr angle smaller in London summer than London winter', () => {
    const summer = getAngles('2024-06-21', 51.5, -0.1).fajrAngle;
    const winter = getAngles('2024-12-21', 51.5, -0.1).fajrAngle;
...
describe('Asr convention', () => {
  it('Hanafi Asr later than Shafii at multiple locations', () => {
    ...
    for (const [lat, lng, tz] of locations) {
      const tS = getTimes('2024-06-21', lat, lng, tz, 0, 15, 1013.25, false);
      const tH = getTimes('2024-06-21', lat, lng, tz, 0, 15, 1013.25, true);
  ...
  it('Hanafi-Shafii difference 20-85 min at typical latitudes', () => {
    const t = getTimes('2024-06-21', 40.7, -74.0, -4);
    const tH = getTimes('2024-06-21', 40.7, -74.0, -4, 0, 15, 1013.25, true);
...
describe('Atmospheric parameters', () => {
  it('Higher elevation brings Sunrise earlier', () => {
    const t0 = getTimes('2024-06-21', 40.7, -74.0, -4, 0);
    const t1 = getTimes('2024-06-21', 40.7, -74.0, -4, 2000);
  ...
  it('Temperature and pressure accepted without error', () => {
    const t = getTimes('2024-06-21', 40.7, -74.0, -4, 100, 5, 950);
    assert(isFinite(t.Sunrise));
  });

  it('Extreme cold reduces refraction slightly', () => {
    const tHot = getTimes('2024-06-21', 40.7, -74.0, -4, 0, 40, 1013.25);
    const tCold = getTimes('2024-06-21', 40.7, -74.0, -4, 0, -20, 1013.25);
...
describe('calcTimes — formatting', () => {
  it('returns HH:MM:SS strings', () => {
    const t = calcTimes('2024-06-21', 40.7, -74.0, -4);
...
describe('getTimesAll — method comparison', () => {
  it('returns Methods map with 14 entries', () => {
    const t = getTimesAll('2024-06-21', 40.7, -74.0, -4);
  ...
  it('MWL Isha at London summer may be NaN (18° fails)', () => {
    const t = getTimesAll('2024-06-21', 51.5, -0.1, 1);
    assert(typeof t.Methods.MWL[1] === 'number');
  });

  it('UAQ Isha = Maghrib + 90min', () => {
    const t = getTimesAll('2024-06-21', 21.4, 39.8, 3);
  ...
  it('Qatar Isha = Maghrib + 90min', () => {
    const t = getTimesAll('2024-06-21', 25.3, 51.5, 3);
  ...
  it('higher-angle methods have earlier Fajr', () => {
    const t = getTimesAll('2024-06-21', 1.3, 103.8, 8);
...
describe('calcTimesAll', () => {
  it('returns formatted strings', () => {
    const t = calcTimesAll('2024-06-21', 40.7, -74.0, -4);
    const timeRe = /^\d{2}:\d{2}:\d{2}$/;
  ...
  it('N/A for unreachable events', () => {
    const t = calcTimesAll('2024-06-21', 58.0, 25.0, 3);
...
describe('Date coverage', () => {
  ...
  it('works on Feb 29 in leap year', () => {
    const t = getTimes('2024-02-29', 40.7, -74.0, -5);
    assert(isFinite(t.Fajr), 'Feb 29 Fajr not finite');
  });

  it('works on Dec 31', () => {
    const t = getTimes('2024-12-31', 40.7, -74.0, -5);
    assert(isFinite(t.Sunrise));
  });

  it('works on Jan 1', () => {
    const t = getTimes('2024-01-01', 40.7, -74.0, -5);
    assert(isFinite(t.Sunrise));
  });

  // The two equinoxes have near-identical day length but NOT near-identical clock times.
  // The equation of time runs about -7.5 min in late March and about +7 min in late
  // September, so NYC sunrise lands at 06:58 EDT in March and 06:44 EDT in September — a
  // real 14-minute gap, not an error. An earlier version of this test asserted the two
  // were within 15 minutes of each other, which happened to hold only by a minute and
  // encoded a symmetry the equation of time does not permit.
  it('both equinoxes match the real clock times, EoT gap included', () => {
    const t1 = getTimes('2024-03-20', 40.7, -74.0, -4);
    const t2 = getTimes('2024-09-22', 40.7, -74.0, -4);
    assert(approx(t1.Sunrise, hm(6, 58), 0.05), `Spring equinox Sunrise ${t1.Sunrise}`);
    assert(approx(t2.Sunrise, hm(6, 44), 0.05), `Autumn equinox Sunrise ${t2.Sunrise}`);
    // Day length is what the equinoxes actually share. It runs a little OVER 12 hours,
    // not exactly 12: sunrise and sunset are defined at the sun's upper limb rather than
    // its centre, and atmospheric refraction lifts the disc into view early and holds it
    // there late. Together those add roughly ten minutes, so ~12h10m is the correct
    // equinox day length at this latitude, not a rounding error.
    const day1 = t1.Maghrib - t1.Sunrise;
    const day2 = t2.Maghrib - t2.Sunrise;
    assert(day1 > 12.05 && day1 < 12.3, `Spring equinox day length ${day1}h`);
    assert(day2 > 12.05 && day2 < 12.3, `Autumn equinox day length ${day2}h`);
    assert(Math.abs(day1 - day2) < 0.05, `Equinox day lengths differ by ${(day1 - day2) * 60} min`);
  });
});
...
describe('Winter scenarios', () => {
  it('London winter — all core times finite', () => {
    const t = getTimes('2024-12-21', 51.5, -0.1, 0);
  ...
  it('Moscow winter — all core times finite', () => {
    const t = getTimes('2024-12-21', 55.8, 37.6, 3);
    for (const f of ['Fajr','Sunrise','Noon','Dhuhr','Asr','Maghrib','Isha']) {
      assert(isFinite(t[f]), `${f}=${t[f]}`);
    }
  });
  ...
  it('Oslo winter — Noon finite', () => {
    const t = getTimes('2024-12-21', 59.9, 10.8, 1);
...
describe('Input validation', () => {
  ...
  it('rejects longitude > 180', () => {
    assert.throws(() => getTimes('2024-06-21', 0, 181, 0), { name: 'RangeError' });
  });
  ...
  it('rejects timezone > 14', () => {
    assert.throws(() => getTimes('2024-06-21', 0, 0, 15), { name: 'RangeError' });
  });

  it('rejects NaN latitude', () => {
    assert.throws(() => getTimes('2024-06-21', NaN, 0, 0), { name: 'RangeError' });
  });

  it('rejects Infinity longitude', () => {
    assert.throws(() => getTimes('2024-06-21', 0, Infinity, 0), { name: 'RangeError' });
  });

  it('getTimesAll also validates', () => {
    assert.throws(() => getTimesAll('2024-06-21', 91, 0, 0), { name: 'RangeError' });
  });
});
